    ...
    <div class="flex w-max flex-col gap-3 text-fg-default">
      ...
      <div class="flex w-40 flex-col">
        <button
          ...
          :key="s.label"
          type="button"
          class="flex items-center border-thick bg-surface-default px-4 py-2 text-left font-body text-sm cursor-pointer shadow-flat transition-colors transition-press hover:bg-surface-subtle hover:shadow-low"
          :class="s.position"
        >
          {{ s.label }}
        </button>
