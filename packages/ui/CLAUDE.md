...
- `Could not capture a stable screenshot` failures correlate with full-suite CPU contention, not the first run of a session. The matcher's stability retry takes screenshots back-to-back with no interval, comparing consecutive pairs until two match, inside a fixed timeout window (`toMatchScreenshot.timeout` in `vitest.config.ts`, raised past the 5000ms default). With ~68 browser-mode files sharing one chromium instance, each capture round is slower under load, so fewer retries fit before the window closes. Rerunning a flake in isolation "fixes" it only because the contention is gone, not because anything warmed up. If this recurs, raise the timeout further before suspecting the component.
